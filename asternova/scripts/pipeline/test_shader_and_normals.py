...
# Flower & Ribbon placement on left side
...
# 4. Create Aster NPR Pearl White Cel Hair Shader
hair_mat = bpy.data.materials.new(name='Aster_Hair_NPR_PearlWhite')
...
links.new(diffuse.outputs['BSDF'], s2rgb_diff.inputs['Shader'])

cel_ramp = nodes.new('ShaderNodeValToRGB')
cel_ramp.color_ramp.interpolation = 'EASE'
# Shadow: Pale Lavender-Purple (#dcd8ee)
cel_ramp.color_ramp.elements[0].position = 0.44
cel_ramp.color_ramp.elements[0].color = (0.86, 0.84, 0.93, 1.0)
# Lit: Pearl White (#f2f4fc)
cel_ramp.color_ramp.elements[1].position = 0.52
cel_ramp.color_ramp.elements[1].color = (0.95, 0.96, 0.99, 1.0)
links.new(s2rgb_diff.outputs['Color'], cel_ramp.inputs['Fac'])

# Glossy Specular Angel Ring
...
angel_ramp.color_ramp.elements[0].position = 0.55
angel_ramp.color_ramp.elements[0].color = (0.0, 0.0, 0.0, 1.0)
# Angel ring tint: Celestial Cool Ice Blue (#b0e0ff)
angel_ramp.color_ramp.elements[1].position = 0.78
angel_ramp.color_ramp.elements[1].color = (0.45, 0.68, 0.95, 1.0)
...
# 6. Setup Lighting for high-fidelity evaluation
...
cam.data.lens = 70
bpy.context.scene.render.filepath = os.path.join(out_dir, 'test_npr_hair_front.png')
bpy.ops.render.render(write_still=True)

# View 2: Back
cam.location = (0, 0.9, 1.35)
cam.rotation_euler = (math.radians(90), 0, math.radians(180))
bpy.context.scene.render.filepath = os.path.join(out_dir, 'test_npr_hair_back.png')
bpy.ops.render.render(write_still=True)

# View 3: Side strong light (testing normal smoothness & angel ring)
key_light.rotation_euler = (0.3, 0.8, 1.2) # Strong side angle
cam.location = (-0.6, -0.5, 1.48)
cam.rotation_euler = (math.radians(90), 0, math.radians(-50))
bpy.context.scene.render.filepath = os.path.join(out_dir, 'test_npr_hair_sidelight.png')
bpy.ops.render.render(write_still=True)

print("ALL 3 NPR HAIR TESTS RENDERED")
