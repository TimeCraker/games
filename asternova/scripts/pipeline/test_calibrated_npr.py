...
cel_ramp.color_ramp.interpolation = 'LINEAR'
# Shadow color: Rich Lavender-Violet Shadow (#9a94b8)
cel_ramp.color_ramp.elements[0].position = 0.28
cel_ramp.color_ramp.elements[0].color = (0.58, 0.55, 0.68, 1.0)
# Sub-shadow transition: Light Lavender (#d0cce2)
el_mid = cel_ramp.color_ramp.elements.new(0.38)
el_mid.color = (0.80, 0.78, 0.88, 1.0)
# Lit color: Pearl White (#f2f4fc)
cel_ramp.color_ramp.elements[2].position = 0.48
cel_ramp.color_ramp.elements[2].color = (0.95, 0.96, 0.99, 1.0)
...
fresnel_ramp.color_ramp.elements[0].position = 0.75
fresnel_ramp.color_ramp.elements[0].color = (0.0, 0.0, 0.0, 1.0)
fresnel_ramp.color_ramp.elements[1].position = 0.98
...
mix_rim.inputs['Factor'].default_value = 0.35
...
cam.data.lens = 70
bpy.context.scene.render.filepath = os.path.join(out_dir, 'test_calib_front.png')
bpy.ops.render.render(write_still=True)

# Back view (light rotated to illuminate back with beautiful side-shadows)
...
bpy.context.scene.render.filepath = os.path.join(out_dir, 'test_calib_back.png')
bpy.ops.render.render(write_still=True)

# 3/4 Side view with strong side light to show angel ring & normal smoothness
key_light.rotation_euler = (math.radians(30), math.radians(20), math.radians(85))
cam.location = (-0.6, -0.5, 1.48)
cam.rotation_euler = (math.radians(90), 0, math.radians(-50))
bpy.context.scene.render.filepath = os.path.join(out_dir, 'test_calib_side.png')
bpy.ops.render.render(write_still=True)

print("CALIBRATED TESTS RENDERED")
