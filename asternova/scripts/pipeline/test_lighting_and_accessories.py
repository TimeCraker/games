...
em.inputs['Color'].default_value = (0.22, 0.20, 0.30, 1.0)
...
# Shadow: Soft Lavender Shadow (#bdb8d6)
cel_ramp.color_ramp.elements[0].position = 0.35
cel_ramp.color_ramp.elements[0].color = (0.74, 0.72, 0.84, 1.0)
# Lit: Pearl White (#f2f4fc)
cel_ramp.color_ramp.elements[1].position = 0.45
...
angel_ramp.color_ramp.elements[0].position = 0.50
angel_ramp.color_ramp.elements[0].color = (0.0, 0.0, 0.0, 1.0)
# Ice Cyan-Blue Angel Ring
angel_ramp.color_ramp.elements[1].position = 0.72
angel_ramp.color_ramp.elements[1].color = (0.55, 0.80, 1.0, 1.0)
...
mix_angel.inputs['Factor'].default_value = 0.65
...
gem.inputs['Color'].default_value = (0.92, 0.78, 0.48, 1.0) # Champagne Gold
...
pem.inputs['Color'].default_value = (0.96, 0.97, 1.0, 1.0) # Pearl White
...
rem.inputs['Color'].default_value = (0.50, 0.74, 0.96, 1.0) # Sky Blue
...
# 5. Crisp NPR Lighting Setup
bpy.context.scene.world.color = (0.12, 0.12, 0.16) # Darker ambient so cel shadows pop!
...
# Back Rim light for hair definition
rim_light = bpy.data.objects.get('RimLight')
if not rim_light:
    rdata = bpy.data.lights.new('RimLight', type='SUN')
    rim_light = bpy.data.objects.new('RimLight', rdata)
    bpy.context.scene.collection.objects.link(rim_light)
rim_light.data.energy = 2.5
rim_light.data.color = (0.85, 0.92, 1.0)
rim_light.rotation_euler = (-0.6, 0.2, 2.7)
...
cam.data.lens = 70
bpy.context.scene.render.filepath = os.path.join(out_dir, 'test_crisp_front.png')
bpy.ops.render.render(write_still=True)

# Back view
cam.location = (0, 0.9, 1.35)
cam.rotation_euler = (math.radians(90), 0, math.radians(180))
bpy.context.scene.render.filepath = os.path.join(out_dir, 'test_crisp_back.png')
bpy.ops.render.render(write_still=True)

# 3/4 Side view
cam.location = (-0.6, -0.5, 1.48)
cam.rotation_euler = (math.radians(90), 0, math.radians(-50))
bpy.context.scene.render.filepath = os.path.join(out_dir, 'test_crisp_side.png')
bpy.ops.render.render(write_still=True)

print("CRISP LIGHTING TESTS RENDERED")
