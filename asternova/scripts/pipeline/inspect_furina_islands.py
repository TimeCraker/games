...
faces = [f for f in bm.faces if f.material_index == 10]
    ...
    while q:
        ...
        for e in curr.edges:
            for lf in e.link_faces:
                if lf.material_index == 10 and lf not in visited:
                    visited.add(lf); q.append(lf)
    islands.append(isl)

print(f"FURINA MAT 10 (髮): {len(faces)} faces, {len(islands)} islands")
