...
> ⚠️ **状态提示：一期已封存归档（FROZEN）**  
> 依据 2026-08-31 重大架构定案（详见 [BLUEPRINT.md](../../docs/BLUEPRINT.md) 与 [architecture.md](../../docs/architecture.md)），本项目当前确立 **Godot 4.7 单机优先 + 房主联机（Steam 数据中继 / 零自建服务器）** 路线。原 Go 中心服务器与 Unity 前端基建已全面一期封存，本文档仅作为二期大型 MMO 重启时的历史资产与设计参考，当前开发活动严禁基于本路线推进。
