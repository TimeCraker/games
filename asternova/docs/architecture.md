...
| 渲染器 | **全端 Compatibility（2026-08-31 定案）** | 三端一套 shader 同效果；二次元 toon 不依赖 Forward+ 的高级特效（那是写实向吃的）；Web 导出稳定、包体小启动快；Windows 高档画质未来可单独升 Forward+（一期不做） |
...
- **非房主客户端职责（2026-08-31 修订：分层预测——纯「零预测」对动作游戏手感不可用，客人延迟 ≈100-150ms 会「发飘」）**：
  ...
  - **插值统一规则**：客人端关闭 Godot 4.5 本地 3D 物理插值，视觉统一走网络快照插值，避免双插值打架。

- **联机三条语义（2026-08-31 定案，防实现时 agent 自行拍板）**：① 房主掉线 = 局终，按当前进度结算（断线恢复局归二期）② 客人断线重连窗口 60s，重连补发全量快照 ③ 存档：局内进度以房主为准，局末各客户端写各自本地（Steam 云存档 M3 决策接不接）。
...
│   插槽: WS(现行) / SteamTransport│ (二期可整体切换到 │
│        / ENet UDP(M4)           │  中心服务器)     │
└─────────────────────┘        └──────────────────┘
```

- **接口先行**（M2）：`Transport` 抽象 + **fake transport 回环测试**为接口验收标准；回环必须含**延迟 / 丢包注入的混沌变体**（纯回环不丢包，插值与超时逻辑测不到）。接口不关心对端是谁——一期接房主，二期接中心服务器。
- **连接策略六级降级（2026-08-31 定案，客户端自动逐级尝试并显示进度）**：① 局域网发现 → ② IPv6 直连（国内家庭分配率高）→ ③ UPnP 端口映射 → ④ UDP 打洞（同运营商成功率高）→ ⑤ **Steam 数据中继（Valve 免费，GodotSteam 接入，Steam 版玩家主路径）** → ⑥ 自建中继（仅官网直发版 / Web 版兜底，后置评估）。
- **WS 先行**：房主联机首版 WebSocket；**快照与事件分两条 WS 连接**（TCP 队头阻塞会放大弱网快照抖动，分连隔离）。M2 联机验收限定局域网 / 直连环境（WS 是 TCP，跨宽带打洞成功率低属预期）。
