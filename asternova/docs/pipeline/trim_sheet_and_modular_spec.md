...
## 六、 Decals & Signage 参数化贴花与发光标识制作规范（✅ 2026-09-16 定案）

在以光映广场为代表的现代机能二次元街区中，道路警示标线（禁停红叉、停车编号、黄黑斜纹防撞条、减速带、斑马线）与商铺发光招牌是**提供生活烟火气、城市秩序感与视觉构图锚点的核心要素**。

### 1. PIL / Vector 矢量贴图生产管线（毫秒级 2K/4K 零噪点生成）
- **杜绝三维几何硬拼**：严禁在三维视口中用多边形网格去切字捏标线（极易产生布线混乱与拉伸锯齿）；
- **Python PIL (Pillow) 自动化脚本生成**：
  - **分辨率与精度**：统一生成 `2048 × 2048` 无损 RGBA 贴图，线条边缘平滑无压缩色块；
  - **几何参数化排版**：利用脚本数学坐标绘制规整几何体（如粗红叉 Line width 70px、高对比度黄黑 45° 警示斜纹、精准居中排版的日式/英文排版）；
  - **通道分离**：严格导出带透明 Alpha 通道的 8-bit PNG，以及用于灯箱与霓虹招牌的 Emission 发光 Mask。

### 2. 贴花材质与着色器架构
- **材质节点分层（Decal Material）**：
  - **Albedo & Alpha**：BaseColor 直连，Alpha 驱动透明裁切（Cutout 阈值 `0.5`，杜绝边缘半透明脏边）；
  - **发光通道（Emission）**：商铺霓虹灯箱与夜间反光标识统一将 Emission 强度标定在 `2.0 ~ 4.5`，在 AgX 色彩管理下呈现高饱和柔和滚降，不炸成死白光球；
  - **粗糙度（Roughness）**：道路标线漆面标定在 `0.45 ~ 0.55`（比湿润石板略干略毛糙，凸显漆面与石材的真实材质差异）；
- **进阶微表面与物理咬合（Advanced Quality）**：
  - **法线厚度微联动（Normal Relief）**：利用标线边缘生成 `0.5mm ~ 1.0mm` 漆面微浮雕法线，低角度光照下呈现真实的道路刷漆边缘高光；
  - **微风化剥落（Decal Wear & Triplanar）**：叠加二阶微噪波对 Alpha 边缘进行微量剔除（Clip/Erosion），还原市政道路受车辆碾压的自然剥落与轻微划痕，彻底根除“漂浮贴纸”廉价感。
