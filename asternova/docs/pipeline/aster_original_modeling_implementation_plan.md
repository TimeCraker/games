...
| **网格规整与拓扑** | **Blender 四边面重拓扑 (Quad Remesh)** | Agent 将原始 10~20 万面高模重拓扑为 **6.0 万 ~ 8.5 万三角面**（对标《终末地》《鸣潮》PC 旗舰高精档）的标准游戏四边面资产；LOD1 自动减面至 2.5 万 ~ 3.5 万面；面部环形拓扑对齐、开挖裙底与身体保持物理隔离。 |
...
- **任务目标**：
  ...
  2. 拆解分层部件：
     - `Head_Mesh`（面部与头部基座，8k~12k 面）
     - `Hair_Mesh`（前刘海、鬓角侧发、后腰长发，15k~22k 面，进行法线球化与分束）
     - `Body_Skin_Mesh`（锁骨、天鹅颈、手臂、修长双腿，8k~12k 面）
     - `Costume_Mesh`（立领衬衫、维多利亚泡泡袖、高腰封、双层百褶裙，30k~40k 面）
     - `Acc_Shoes_Mesh`（胸前缎带胸针、玛丽珍皮鞋与蕾丝短袜，3k~5k 面）
  3. 执行 Quad Remeshing，输出三角面控制在 **60,000 ~ 85,000 面**（High 档）与 **25,000 ~ 35,000 面**（LOD1 档）的标准四边面资产。
- **阶段 1 验收交付物**：
