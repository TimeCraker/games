...
- **旧 AI 规则文件不作行为依据**：三份 `.cursorrules` 已删除（2026-08-30）；残留 `.roo/rules/*`（backend_rules.md 是旧 Cursor 规则的原样复制）仍属 Cursor/Roo 时代产物，含过时上下文（Unity 客户端、Godot 4.3、"新增代码标记包围"）。有效规则 = 全局 rules + 本文件 + docs/ 蓝图。
