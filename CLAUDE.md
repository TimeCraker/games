...
5. **渲染（2026-09-06 统一定案）**：渲染器锁定 **Forward+ (Vulkan Clustered)**，以《明日方舟：终末地》为最高美学与工业标杆（辅以《鸣潮》《崩铁》，坚决排除绝区零高饱和油腻风格），采用 **AgX 色调映射（模式 4）** + 终末地级双半球冷峻天光 + 全分辨率 SSAO + SSR + PBR 场景 vs NPR 角色双管线；渲染帧率与模拟解耦——模拟 60Hz 固定步长，渲染 120 起步上不封顶（低档锁 60 / 中档 120 / 高档默认解锁至显示器上限）。美术资产生成的一切约束以 `asternova/docs/STYLE.md` 为准，**先出验证件给用户过目再批量生产**。
